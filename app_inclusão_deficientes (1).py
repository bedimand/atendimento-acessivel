# -*- coding: utf-8 -*-
"""app_inclusão_deficientes.ipynb
...
BRAND = "FLEX CARE"
TAGLINE = "Inclusão e Acessibilidade"
COR_ESCURA = "#0E3A47"
COR_DESTAQUE = "#2EC4B6"
COR_SUCESSO = "#2E7D32"
COR_ALERTA = "#C62828"
...
st.set_page_config(page_title=f"{BRAND} • Agendamento", page_icon="🩺", layout="wide")
...
with c2:
    st.markdown(
        ...
        <div class="banner" data-tts="1" aria-label="{BRAND} Agendamento. {TAGLINE}">
          <div class="titulo">{BRAND} • Plataforma</div>
...
def db_stats():
    with get_conn() as con:
        ...
        cur.execute("SELECT COALESCE(MAX(created_at), '') FROM bookings"); last_created = cur.fetchone()[0] or "—"
...
def calc_custo(sol, pacientes, medicos, faixas):
    ...
    for i, p in enumerate(pacientes):
        ...
        if med_faixa[(med["nome"], faixa)] > 1:
            custo += PESO_OVER
            rec_status["aviso_med"] = f"Médico {med['nome']} ocupado"
...
if perfil == "Agendamento":
    tts_texto("Agendamento acessível: escolha especialidade, faixa e tipo. Podemos sugerir alternativa se necessário.")
elif perfil == "Triagem":
    tts_texto("Triagem de pronto-socorro: calcule o grau de urgência.")
else:
    tts_texto("Consulta do médico: visualize agenda e detalhes de pacientes.")
...
if perfil == "Agendamento":
    ...
    with st.form("form_agendamento"):
        ...
        faixa_sel = st.selectbox("Faixa de horário", faixas_horarios)
        tipo_sel = st.radio("Tipo de consulta", tipo_consulta, horizontal=True)
        acc_sel = st.multiselect("Acessibilidades necessárias (opcional)", acessibilidades)
    ...
    if confirmar:
        ...
        if chosen_doc:
            ...
            with get_conn() as con:
                ...
                con.commit()
            st.success(f"Agendado com sucesso: {dia} • {faixa_sel} • {chosen_doc}")
            ...
            if sug:
                ...
                st.warning(f"Não foi possível na opção escolhida. Sugestão: {dia2} • {faixa2} com {doc2}.")
                tts_texto("Não foi possível. Sugestão de novo horário disponível exibida.")
            else:
                st.error("Não encontramos alternativa nos próximos 7 dias considerando as restrições informadas.")
                tts_texto("Sem alternativas nos próximos sete dias considerando suas restrições.")
...
if perfil == "Triagem":
    ...
    with st.form("form_triagem"):
        ...
        with col1:
            ...
            hr = st.number_input("Frequência cardíaca (bpm)", 0, 250, 85)
        with col2:
            rr = st.number_input("Frequência respiratória (irpm)", 0, 60, 16)
            spo2 = st.number_input("Saturação O₂ (%)", 0, 100, 97)
            sbp = st.number_input("PA sistólica (mmHg)", 0, 250, 120)
            bleeding = st.selectbox("Sangramento", ["nenhum", "leve", "moderado", "grave"])
            avpu = st.selectbox("Consciência (AVPU)", ["alerta", "verbal", "dor", "inconsciente"])
        with col3:
            chest_pain = st.checkbox("Dor torácica", value=False)
            dyspnea = st.checkbox("Dispneia", value=False)
            dehydration = st.checkbox("Sinais de desidratação", value=False)
            comorb = st.number_input("Nº de comorbidades (0+)", 0, 10, 0)
            pregnancy = st.checkbox("Gestante?", value=False)
            preg_weeks = st.number_input("Semanas de gestação (se aplicável)", 0, 45, 0)
            onset_hours = st.number_input("Horas desde o início dos sintomas", 0, 168, 24)
        notes = st.text_area("Observações (opcional)")
        usar_ml = st.checkbox("Usar modelo ML experimental (fallback para heurística se indisponível)", value=False)
        esp_enc = st.selectbox("Especialidade de encaminhamento (se salvar)", ["pediatria", "cardiologia", "clínico geral", "dermatologia"], index=2)
        salvar = st.checkbox("Salvar caso no banco (como paciente em PS)", value=False)
        enviar = st.form_submit_button("Calcular urgência")
    if enviar:
        tri_dict = dict(
            age=age,
            sex=sex,
        ...
        if usar_ml:
            try:
                ...
                fonte = "Árvore de decisão (ML) + fallback"
            except Exception:
                pass
        st.info(f"Grau de urgência: **{urg_final}/5** ({fonte})")
        if urg_final >= 4:
            st.error("Atendimento prioritário recomendado.")
        elif urg_final == 3:
            st.warning(" Avaliação em breve (prioridade intermediária).")
        else:
            st.success(" Caso não crítico, pode aguardar.")
        tts_texto(f"Urgência calculada: {urg_final} de 5.")
        if salvar:
            dia = date.today().isoformat()
            periodo = faixa_periodo[random.choice(faixas_horarios)]
            paciente = {
                "data": dia,
                "esp": esp_enc if esp_enc != "clínico geral" else "pediatria",
                "acc": [],
                "periodo": periodo,
                "tipo": "presencial",
                "urg": urg_final,
...
if perfil == "Consulta":
    st.subheader("Agenda do médico e prontuário sintético")
    med_nomes = [m["nome"] for m in medicos]
    med_sel = st.selectbox("Médico", med_nomes)
    df_ag = bookings_df()
        ...
        with colA:
            ...
            if tri_p:
                vitais = {
                    ...
                    "Consciência": tri_p.get("consciousness"),
                    "Dor torácica": tri_p.get("chest_pain"),
                    "Dispneia": tri_p.get("dyspnea"),
                    "Desidratação": tri_p.get("dehydration"),
                    "Comorbidades": tri_p.get("comorb"),
                    "Gestação (semanas)": tri_p.get("pregnancy_wks"),
                    "Início (h)": tri_p.get("onset_hours"),
                    "Observações": tri_p.get("notes"),
                }
                st.json(vitais)
            else:
...
with st.expander("Limpar pacientes (cuidado!)"):
    if st.button("Apagar todos os pacientes"):
        clear_patients()
        st.warning("Pacientes excluídos. Atualize a página para ver a lista vazia.")
        tts_texto("Todos os pacientes foram excluídos")
...
st.caption(f"DB: **{DB_PATH}** • Último agendamento criado em: {_stats['last']}")

if vlibras_enabled:
    enable_vlibras()
    tts_texto("VLibras ativado. Use o botão azul para traduzir para Libras.")
